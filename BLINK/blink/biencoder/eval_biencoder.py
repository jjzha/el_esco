...
def main(params):
    ...
    context_vecs = new_data['context_vecs']
    decoded_contexts = []

    for context_ids in context_vecs:
        decoded_context = tokenizer.decode(context_ids, skip_special_tokens=True)
        decoded_contexts.append(decoded_context)

    # Decode candidate_vecs
    candidate_vecs = new_data['candidate_vecs']
    decoded_candidates = []

    for candidate_ids in candidate_vecs:
        sub_k = []
        for cand in candidate_ids:
            decoded_candidate = tokenizer.decode(cand[:params["top_k"]], skip_special_tokens=True)  # Assuming you want to decode the first candidate
            sub_k.append(decoded_candidate)
        decoded_candidates.append(sub_k)

    # Print or use the decoded sequences as needed
    if save_results: 
        ...
        with open(f"{save_data_dir}/output.jsonl", "w") as fout:
            for decoded_context, decoded_candidate in zip(decoded_contexts, decoded_candidates):
                d = {
                    "k": str(params["top_k"]),
